...
public class Afficheur {
	...
	public void afficherReseau(Graphics g, Simulateur simulateur,
		...
		if (simulateur.reqPositionProchaineUrgence() != null) {
			...
			this.afficherProchaineUrgence(g,
	...
	private void afficherNoeuds(Graphics g, ArrayList<Noeud> listeDeNoeuds) {
		...
		for (Noeud noeud : listeDeNoeuds) {
			Position position = m_gestionnaireReseau.reqPositionEnPixel(noeud
					.reqPosition());

			if (position.reqPositionX()
					+ m_gestionnaireReseau.reqPixelParStep() > maxWidth) {

				maxWidth = Math.round(position.reqPositionX() + 3
						* m_gestionnaireReseau.reqPixelParStep());
			}

			if (position.reqPositionY()
					+ m_gestionnaireReseau.reqPixelParStep() > maxHeight) {
				maxHeight = Math.round(position.reqPositionY() + 3
						* m_gestionnaireReseau.reqPixelParStep());
			}
			...
			Ellipse2D.Double circle = new Ellipse2D.Double(a, b, WIDTH_NOEUD,
					WIDTH_NOEUD);
			g2d.draw(circle);
		}
	}
	...
	private void afficherProchaineUrgence(Graphics g, Position position) {
		...
		if (position != null && toggleUrgence) {
			Graphics2D g2d = (Graphics2D) g;
			Image img = m_ImageUrgence.getScaledInstance(WIDTH_NOEUD,
	...
	private void afficherArcs(Graphics g, ArrayList<Arc> listeArcs) {
		...
		for (Arc arc : listeArcs) {
			Position source = m_gestionnaireReseau.reqPositionEnPixel(arc
					.reqNoeudSource().reqPosition());
			Position destination = m_gestionnaireReseau.reqPositionEnPixel(arc
	...
	private void afficherVehicule(Graphics g, Position position,
		...
		if (position != null) {
			...
			g2d.drawImage(
					op.filter(img, null).getScaledInstance(WIDTH_NOEUD * 2,
							WIDTH_NOEUD * 2, Image.SCALE_SMOOTH), (int) a,
					(int) b, (WIDTH_NOEUD * 2) + 2, (WIDTH_NOEUD * 2) + 2, null);

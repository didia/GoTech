...
public class Afficheur {
	...
	private void afficherGrille(Graphics g, CarteGraphique carteGraphique) {
		...
		// carteGraphique.setPreferredSize(new
		// Dimension(Math.round(Default.CARTE_WIDTH*m_zoom),Math.round(Default.CARTE_HEIGHT*m_zoom)));
		// carteGraphique.revalidate();
	...
	private void afficherNoeuds(Graphics g, ArrayList<Noeud> listeDeNoeuds) {
		...
		for (Noeud noeud : listeDeNoeuds) {
			Position position = m_grille
					.reqPositionEnPixel(noeud.reqPosition());
			// System.out.println("Noeud " + position.reqPositionX() + " " +
			// position.reqPositionY());

...
public class Carte {
	...
	public Noeud reqNoeud(Position position) {
		for (Noeud noeud : m_listeDeNoeuds) {
			...
			/*
			 * float x = position.reqPositionX(); float y =
			 * position.reqPositionY(); float c1 =
			 * noeud_position.reqPositionX(); float c2 =
			 * noeud_position.reqPositionY(); float circle = (x - c1) * (x - c1)
			 * + (y - c2) * (y - c2);
			 * 
			 * if (circle <= 10 * 10) { return noeud; }
			 */
